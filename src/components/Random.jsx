...
function Random() {
    ...
    // const mealTitle = random.recipe.title;
    // const mealInstruction = random.recipe.instructions;
    ...
    return (
        <div className='border border-blue-600 m-2 p-2'>
            <div className='grid grid-cols-4 p-8'>
                {random.map((recipe) => {
                    return (
                        <section key={recipe.id} className="m-6">
                            <div className="border rounded-lg overflow-hidden shadow-lg relative h-96 min-h-full mx-6 " key={recipe.id}>
                                ...
                                <p className="font-bold text-xl m-2 absolute z-10 bottom-0 w-full h-2/5 text-center flex justify-center items-center left-1/2 -translate-x-1/2 translate-y-0 text-white">{recipe.title}</p>
                                {/* <div dangerouslySetInnerHTML={{ __html: recipe.instructions }}> */}
                                    {/* {renderInstruction(recipe.instructions)} */}
                                {/* </div> */}
                            </div>
                            <Modal title={recipe.title} instructions={recipe.instructions}/>
                        </section>
                    );
                })}
            </div>
